...
def handle_command(client_socket, data, nickname, current_channel):
    if data.startswith("NICK"):
        ...
        if not nickname:
            client_socket.sendall("f: SERVER_HOST 451 :You have not registered\r\n".encode()) #ERROR_NOTREGISTERED (451)
        else:
            handle_privmsg(client_socket, data, nickname)
...
def handle_privmsg(client_socket, data, nickname ):
    parts = data.split(" ",2)
    if len(parts) < 3:
        client_socket.sendall("461 *PRIVMSG : Parameters are not large enough\r\n".encode())
        return
    ...
    try:
        if target in channels:
            ...
            if nickname not in channels[target]:
                client_socket.sendall(f"{SERVER_HOST} 403 {nickname} {target}: You are not in the channel\r\n".encode())
                return
            
            for user in channels[target]:
                if user != nickname:
                   try: 
                    connected_clients[user].sendall(f":{nickname} PRIVMSG {target} :{message}\r\n".encode())
                   except Exception as e: 
                       
                       print (f"Error sending message to {user}:{e}")
        elif target in connected_clients:
            connected_clients[target].sendall(f":{nickname} PRIVMSG {target} :{message}\r\n".encode())
        
        else:
            client_socket.sendall(f"{SERVER_HOST} 401 {nickname} {target} : No such nick/channel\r\n".encode())
    except Exception as e:
        print(f"Oops looks like an error has occurred while sending the Private message:{e}")
        client_socket.sendall("ERROR : An internal error occured\r\n".encode())
def main():
    start_server()
